...
public class EChartsOption
{
	...
	*/
	...
	protected void onInit() throws Exception
	{
		...
		if (chartAxisModels != null)
		{
			while (chartAxisModels.hasNext())
			{
				...
				if ((StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_TOP, true) == 0)
						|| (StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_BOTTOM, true) == 0))
				{
					xAxisList.add(eChartsAxis);
					continue;
				}
				if ((StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_LEFT, true) == 0)
						|| (StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_RIGHT, true) == 0))
				{
	...
	protected EChartsSeries createSeries(String strType) throws Exception
	{
		...
		if ((StringHelper.compare(strType, IChartSeriesModel.SERIESTYPE_BAR, true) == 0) || (StringHelper.compare(strType, IChartSeriesModel.SERIESTYPE_BAR3D, true) == 0)
				||(StringHelper.compare(strType, IChartSeriesModel.SERIESTYPE_COLUMN, true) == 0) )
		{
	...
	protected EChartsAxis createAxis(IChartAxisModel iChartAxisModel) throws Exception
	{
		if ((StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_LEFT, true) == 0)
				|| (StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_RIGHT, true) == 0)
				|| (StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_TOP, true) == 0)
				|| (StringHelper.compare(iChartAxisModel.getAxisPos(), IChartAxisModel.AXISPOS_BOTTOM, true) == 0))
		{
	...
	public void addTimeCatalog(String strTimeMode,java.util.Date dtBeginTime,java.util.Date dtEndTime)throws Exception
	{
		...
		while(true)
		{
			...
			if(calendar.getTime().getTime()>=dtEndTime.getTime())
				break;
