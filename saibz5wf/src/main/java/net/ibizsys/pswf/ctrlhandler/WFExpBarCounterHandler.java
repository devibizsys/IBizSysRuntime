...
public class WFExpBarCounterHandler extends CounterHandlerBase
{
	...
	public final static String COUNTER_SQL2 = " select wfstepname ,wfworkflowid,pwfstepname,COUNT(*) AS CNT from " +
			"	( 	SELECT t2.wfstepname,t1.wfworkflowid,t5.wfstepname as pwfstepname  FROM T_SRFWFWORKLIST t1 " +
			"	inner join t_srfwfinstance t3 on t3.wfinstanceid = t1.wfinstanceid" +
			"	inner join t_srfwfinstance t4 on t3.pwfinstanceid = t4.wfinstanceid" +
			"	inner join t_srfwfstep t2 on t1.wfstepid = t2.wfstepid " +
			"   inner join t_srfwfstep t5 on t4.activestepid = t5.wfstepid" +
			"	 WHERE t3.PARALLELINST=1 AND  t1.CANCELFLAG = 0 and t1.WFACTORID=? AND t4.WFWORKFLOWID=? AND t4.USERDATA4=? " +
			"	) a GROUP BY wfstepname,wfworkflowid,pwfstepname  ";
	...
	protected AjaxActionResult onFetch() throws Exception
	{
		...
		for (IEntity dataEntity : list)
		{
			...
			if (StringHelper.compare(strWFStepName, "WFSTEPNAME", true) == 0)
				continue;
