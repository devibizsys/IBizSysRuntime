...
public class WFDataCtrl implements IWFDataCtrl
{
	...
	private WFInstanceService wfInstanceService = null;

	private WFStepDataService wfStepDataService = null;

	private WFStepService wfStepService = null;

	private WFStepActorService wfStepActorService = null;

	private WFStepInstService wfStepInstService = null;

	private WFIAActionService wfIAActionService = null;

	private WFWorkListService wfWorkListService = null;

	private WFUserService wfUserService = null;

	private WFTmpStepActorService wfTmpStepActorService = null;

	private MsgTemplateService msgTemplateService = null;
	...
	private boolean bUserCandidate = true;

	private boolean bNoLogSystem = true;
	...
	public WFInstance getWFInstance(IWFActionContext2 iWFActionContext, WFInstance wfInstance, boolean bTryMode) throws Exception
	{
		...
		if (StringHelper.isNullOrEmpty(strWFInstanceId))
		{
			...
			if (wfInstanceList.size() == 0)
			{
				...
				{
					throw new WFException(WFErrors.E_000037,StringHelper.format("指定工作流实例不存在"));
				}
			}
			...
			if (wfInst2 == null)
			{
				...
				{
					throw new WFException(WFErrors.E_000036,StringHelper.format("指定工作流实例已经关闭，无法操作"));
				}
			}
	...
	protected WFUser reCalcRecvWorkWFUser(String strWFUserId) throws Exception
	{
		...
		{
			throw new WFException(WFErrors.E_000034,StringHelper.format("找不到指定工作流用户[%1$s]",strWFUserId));
		}
		if(!DataObject.getBoolValue(wfUserCur.getValidFlag(),true))
		{
			throw new WFException(WFErrors.E_000033,StringHelper.format("工作流用户[%1$s]没有启用",wfUserCur.getWFUserName()));
		}
		...
		catch (Exception ex)
		{
			...
			if (ex instanceof ErrorException)
			{
				...
				throw new WFException(WFErrors.E_000035,StringHelper.format("工作流用户[%1$s]不接收工作，且没有指定待办用户",wfUserCur.getWFUserName()));
			}
			else
				throw ex;
		}
