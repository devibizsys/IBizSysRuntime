...
public abstract class WFServiceBase implements IWFService
{
	...
	public WFActionResult start(WFActionParam wfParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (wfInstance != null)
				{
					throw new WFException(WFErrors.E_000001,StringHelper.format("数据[%1$s]流程已经存在",iEntity.get("srfdatainfo")));
				}
				...
				{
					throw new WFException(WFErrors.E_000002,StringHelper.format("数据[%1$s]流程启动检查失败", iEntity.get("srfdatainfo")));
				}
	...
	public WFActionResult submit(WFActionParam wfParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				{
					throw new WFException(WFErrors.E_000003,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行交互处理", iEntity.get("srfdatainfo")));
				}

				// 判断当前步骤是否一致
				if (StringHelper.compare(wfActionParam.getStepId(), wfInstance.getActiveStepName(), true) != 0)
				{
					throw new WFException(WFErrors.E_000004,StringHelper.format("提交实例[%1$s]处理步骤[%2$s]与当前步骤[%3$s]不一致", iEntity.get("srfdatainfo"), wfActionParam.getStepId(), wfInstance.getActiveStepName()));
				}
				...
				if (curWFProcessModel instanceof IWFInteractiveProcessModel)
				{
					...
					if (!StringHelper.isNullOrEmpty(wfActionContext.getUserTag2()))
					{
						...
						{
							throw new WFException(WFErrors.E_000005,StringHelper.format("当前用户不在指定的代办用户范围"));
						}
						...
						if (!bTestOK)
						{
							throw new WFException(WFErrors.E_000006,StringHelper.format("当前用户不能为工作用户代办指定事项"));
						}
					}
					...
					if (StringHelper.isNullOrEmpty(wfActionParam.getConnection()))
					{
						if (wfActionParam.isTestMode())
						{
							...
							{
								throw new WFException(WFErrors.E_000007,StringHelper.format("没有找到任何交互操作"));
							}
				...
				if (StringHelper.compare(strNextCondition, "UDF", true) == 0)
				{
					...
					if (conds.length < 2)
					{
						throw new WFException(WFErrors.E_000018,StringHelper.format("没有指定用户数据属性"));
					}
				...
				{
					throw new WFException(WFErrors.E_000019,StringHelper.format("暂时不支持自定义连接"));
	...
	public WFActionResult close(WFActionParam wfParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000020,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法再次关闭",iEntity.get("srfdatainfo")));
				}
				...
				{
					throw new WFException(WFErrors.E_000021,StringHelper.format("数据[%1$s]流程不允许关闭", iEntity.get("srfdatainfo")));
				}
	...
	public WFActionResult restart(WFActionParam wfParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				{
					throw new WFException(WFErrors.E_000022,StringHelper.format("数据[%1$s]流程不允许重启", iEntity.get("srfdatainfo")));
				}
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				while (!StringHelper.isNullOrEmpty(strNext))
				{
					...
					{
						throw new WFException(WFErrors.E_000023,StringHelper.format("当前处理后续为结束处理，不存在交互操作处理"));
					}
					...
					throw new WFException(WFErrors.E_000024,StringHelper.format("无法计算当前处理后续的交互处理"));
				}

				if (nextIAProcessModel == null)
				{
					throw new WFException(WFErrors.E_000025,StringHelper.format("无法获取当前处理后续的交互处理"));
				}
	...
	public WFActionResult rollbackIAAction(WFActionParam wpParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000008,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行撤回处理", iEntity.get("srfdatainfo")));
				}
				...
				{
					throw new WFException(WFErrors.E_000026,StringHelper.format("当前用户不是工作流最后一次操作者，无法进行撤回处理"));
				}

				if (StringHelper.compare(lastWFStepData.getConnectionName(), TAG_SRFWFROLLBACK, true) == 0)
				{
					throw new WFException(WFErrors.E_000027,StringHelper.format("无法对撤回处理进行再撤回"));
				}

				if (StringHelper.compare(lastWFStepData.getConnectionName(), TAG_SRFWFTIMEOUT, true) == 0)
				{
					throw new WFException(WFErrors.E_000028,StringHelper.format("无法对超时处理进行撤回"));
				}
				...
				{
					throw new WFException(WFErrors.E_000029,StringHelper.format("没有指定工作流上一个步骤"));
				}
				...
				{
					throw new WFException(WFErrors.E_000030,StringHelper.format("工作流上一个步骤有多个操作者，无法进行撤回处理"));
				}
	...
	public WFActionResult timeoutIAAction(WFActionParam wpParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000010,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行超时处理", iEntity.get("srfdatainfo")));
				}
				...
				if (!bIAGoto)
				{
					...
					{
						throw new WFException(WFErrors.E_000031,StringHelper.format("指定处理[%1$s]不支持超时处理", wfInstance.getActiveStepName()));
					}

					strTimeoutNext = curWFProcessModel.getTimeoutNext();
					if (StringHelper.isNullOrEmpty(strTimeoutNext))
					{
						throw new WFException(WFErrors.E_000032,StringHelper.format("指定处理[%1$s]没有定义超时路径", wfInstance.getActiveStepName()));
					}
	...
	public WFActionResult resubmitAction(WFActionParam wpParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000011,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行工作转移", iEntity.get("srfdatainfo")));
				}
	...
	public WFActionResult markReadFlag(WFActionParam wpParam) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000009,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行标记已读操作",iEntity.get("srfdatainfo")));
				}
	...
	protected void internalExecute(WFActionContext wfActionContext, IWFProcessModel iWFProcessModel) throws Exception
	{
		...
		if (iWFProcessModel == null)
		{
			throw new WFException(WFErrors.E_000012,"没有找到流程开始节点");
		}
		...
		while (true)
		{
			...
			{
				throw new WFException(WFErrors.E_000013,StringHelper.format("[%1$s][%2$s]执行后，没有指定后续节点", iWFProcessModel.getLogicName(), iWFProcessModel.getName()));
			}
			...
			{
				throw new WFException(WFErrors.E_000014,StringHelper.format("处理已经超过[%1$s]次，系统中断", wfActionContext.getMaxLoopCount()));
			}
...
//									}
	...
	protected void internalFinishProcess(WFActionContext wfActionContext, IWFProcessModel iWFProcessModel) throws Exception
	{
		...
		{
			throw new WFException(WFErrors.E_000015,"当前处理步骤无效");
		}
	...
	protected void internalExecuteInteractiveProcess(WFActionContext wfActionContext, IWFProcessModel iWFProcessModel) throws Exception
	{
		...
		if (StringHelper.isNullOrEmpty(wfActionContext.getCurNext()))
		{
			throw new WFException(WFErrors.E_000016,StringHelper.format("[%1$s]执行后，没有指定后续节点", iWFProcessModel.getName()));
		}
	...
	public WFActionResult submitEmbedWorkflow(WFActionParam wpParam, WFInstance childWFInstance, IEntity dataEntity, boolean bNormalClose) throws Exception
	{
		...
		return this.doWFServiceWork(new IWFServiceWork()
		{
			...
			public WFActionResult execute(ITransaction iTransaction) throws Exception
			{
				...
				if (DataObject.getBoolValue(wfInstance.getIsClose(), false))
				{
					throw new WFException(WFErrors.E_000017,StringHelper.format("指定工作流实例[%1$s]已经关闭，无法进行提交处理", wfActionParam.getInstanceId()));
				}
				...
				{
					throw new WFException(WFErrors.E_000004,StringHelper.format("提交实例[%1$s]处理步骤[%2$s]与当前步骤[%3$s]不一致", wfActionParam.getInstanceId(), wfActionParam.getStepId(), wfInstance.getActiveStepId()));
				}
