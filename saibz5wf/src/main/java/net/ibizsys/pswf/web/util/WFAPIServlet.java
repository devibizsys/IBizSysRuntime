...
public class WFAPIServlet extends HttpServletBase
{
	...
	protected AjaxActionResult onProcessAction() throws Exception
	{
		try
		{
			...
			this.getWebContext().setSessionValue(IWebContext.UESRNAME, strUserName);
			...
			if(StringHelper.compare(strCall, ACTION_GETWORKLIST, true)==0)
			{
				...
				sql.append("select t3.WFWORKFLOWID,t3.WFWORKFLOWNAME,t1.WFPLOGICNAME,t4.ACTORID,t1.WFSTEPNAME,t2.USERDATA4,t2.USERDATA ,t1.createdate,t2.wfinstanceid,t2.wfinstancename from T_SRFWFSTEP t1 " +
					" INNER JOIN t_SRFWFINSTANCE t2 on  t2.ACTIVESTEPID = t1.WFSTEPID AND ( t2.ISCLOSE IS NULL OR t2.ISCLOSE=0) "+
					...
					"where t5.WFSTEPDATAID IS NULL AND t4.ACTORID=? order by t1.createdate " );
			...
			if(StringHelper.compare(strCall, ACTION_GETSTEPDATALIST, true)==0)
			{
				...
				//获取我的工作
				StringBuilderEx sql = new StringBuilderEx();
				sql.append("select t1.*,wf1.WFWORKFLOWID from t_srfwfstepdata t1 " +
						" INNER JOIN T_SRFWFINSTANCE wf1 ON t1.WFINSTANCEID = wf1.WFINSTANCEID " +
						" where wf1.USERDATA=? and wf1.USERDATA4=? and (wf1.WFINSTANCEID =?  OR wf1.PWFINSTANCEID =?) order by t1.createdate " );
			...
			if(StringHelper.compare(strCall, ACTION_GETSTEPACTORLIST, true)==0)
			{
				...
				//获取我的工作
				StringBuilderEx sql = new StringBuilderEx();
				sql.append("select t1.* ,wf1.WFWORKFLOWID from t_srfwfstepactor t1 " +
						" INNER JOIN T_SRFWFINSTANCE wf1 ON t1.WFINSTANCEID = wf1.WFINSTANCEID and wf1.ACTIVESTEPID = t1.WFSTEPID " +
						" where wf1.USERDATA=? and wf1.USERDATA4=? and (wf1.WFINSTANCEID =?  OR wf1.PWFINSTANCEID =?) order by t1.createdate " );
			...
			if(StringHelper.compare(strCall, ACTION_GETSTEPACTIONLIST, true)==0)
			{
				...
				//获取我的工作
				StringBuilderEx sql = new StringBuilderEx();
				sql.append("select t1.*  from t_srfwfiaaction t1  " +
						" INNER JOIN T_SRFWFINSTANCE wf1 ON t1.WFSTEPID = wf1.ACTIVESTEPID  " +
						" where wf1.WFINSTANCEID =? order by t1.orderflag " );
