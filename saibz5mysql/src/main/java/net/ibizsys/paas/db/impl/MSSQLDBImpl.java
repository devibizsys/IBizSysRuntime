 ...
 *
 */
public class MSSQLDBImpl extends DatabaseImpl
{
	private static final Log log = LogFactory.getLog(MSSQLDBImpl.class);
	...
	public String getPagingSQL(String strSQL, int nStartPos, int nPageSize, String strMajor, String strMajorDirection, String strMinor, String strMinorDirection)
	{
		...
		if(!StringHelper.isNullOrEmpty(strMajor))
		{
			...
			{
				script.append("SELECT * FROM (Select m1.*,  ROW_NUMBER() over (order by  m1.[%2$s] %3$s)   as SRFROWINDEX from ( select * from (%1$s) pagetemp ) m1 ",//ORDER BY pagetemp.%2$s %3$s
						strSQL, strMajor,strMajorDirection );
			}
			else
			{
				script.append("SELECT * FROM (Select m1.*,  ROW_NUMBER() over (order by  m1.[%2$s] %3$s, m1.[%4$s] %5$s)   as SRFROWINDEX from ( select * from (%1$s) pagetemp ) m1 ",//ORDER BY pagetemp.%2$s %3$s
						strSQL, strMajor,strMajorDirection,strMinor,strMinorDirection );
			}
		}
		else
		{
			script.append("SELECT * FROM (Select m1.*,  ROW_NUMBER() over (order by (select 0))   as SRFROWINDEX from ( select * from (%1$s) pagetemp ) m1 ", //ORDER BY pagetemp.updatedate desc
					strSQL);
		}
		
		script.append(" ) a1 WHERE a1.SRFROWINDEX >= %1$s and a1.SRFROWINDEX < %2$s ",
		nStartPos+1,nStartPos +nPageSize+1);
	...
	public DBCallResult callSql(java.sql.Connection connection,String strCommand, SqlParamList list, int nTimeOut)
	{
		...
		try
		{
			...
			MSSQLDataSetImpl dataSet = new MSSQLDataSetImpl(conn,cstmt);
			while (true)
			{
	...
	/* (non-Javadoc)
	 * @see net.ibizsys.paas.db.IDatabase#getConditionSQL(java.lang.String, int, java.lang.String, java.lang.String, boolean, net.ibizsys.paas.db.SqlParamList)
	 */
	@Override
...
//		}
	...
	protected String getDateTimeConditionSQL(String strFieldName, int nStdDataType, String strCondOp, String strValue, boolean bParam, SqlParamList sqlParamList) throws Exception
	{
		if (!bParam)
		{
			...
			if(StringHelper.compare(strCondOp, ICondition.CONDOP_EQ,true) == 0)
			{
				return StringHelper.format("%1$s = convert (datetime,'%2$s',120)  ",strFieldName,strValue.toUpperCase());
			}
...
//			}
		...
		else
		{
			...
			{
				sqlParamList.add(sqlParam);
				return StringHelper.format("%1$s = convert (datetime,?,120)  ",strFieldName,strValue.toUpperCase());
			}
...
//			}
	...
	/* (non-Javadoc)
	 * @see net.ibizsys.paas.db.impl.DatabaseImpl#getDBObjStandardName(java.lang.String)
	 */
	@Override
