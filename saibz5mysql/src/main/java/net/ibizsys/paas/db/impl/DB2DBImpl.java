 ...
 *
 */
public class DB2DBImpl extends DatabaseImpl
{
	private static final Log log = LogFactory.getLog(DB2DBImpl.class);
	...
	public String getPagingSQL(String strSQL, int nStartPos, int nPageSize, String strMajor, String strMajorDirection, String strMinor, String strMinorDirection)
	{
		...
		if(!StringHelper.isNullOrEmpty(strMajor))
		{
			...
			{
				script.append("SELECT * FROM (Select m1.*, rownumber() over(ORDER BY m1.%2$s %3$s) as SRFROWINDEX from (%1$s ) m1 ",
						strSQL, strMajor,strMajorDirection );
			}
			else
			{
				script.append("SELECT * FROM (Select m1.*, rownumber() over(ORDER BY m1.%2$s %3$s,%4$s %5$s) as SRFROWINDEX from (%1$s ) m1 ",
		...
		script.append(" ) AS a1 WHERE a1.SRFROWINDEX >= %1$s and a1.SRFROWINDEX < %2$s ",
	...
	public DBCallResult callSql(java.sql.Connection connection,String strCommand, SqlParamList list, int nTimeOut)
	{
		...
		try
		{
			...
			DB2DataSetImpl dataSet = new DB2DataSetImpl(conn,cstmt);
			while (true)
			{
    ...
    /**
     * 格式化存储过程调用命令串
     * @param strProcName 过程名称
     * @param nParamCount 参数数量
     * @return 存储过程调用命令串 {?...}
     */
