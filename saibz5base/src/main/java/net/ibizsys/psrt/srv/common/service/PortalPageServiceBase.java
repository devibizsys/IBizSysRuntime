...
/**
 * 实体[PortalPage] 服务对象基类
 */
public abstract class PortalPageServiceBase extends net.ibizsys.psrt.srv.PSRuntimeSysServiceBase<PortalPage> {
    ...
    protected void onBeforeRemove(PortalPage et) throws Exception {
        //判断是否可以删除 关系 门户视图部件 数据
        if(true) {
            PVPartService service = (PVPartService)ServiceGlobal.getService(PVPartService.class,this.getSessionFactory());
            service.testRemoveByPortalPage(et);
        }
        //判断是否可以删除 关系 门户页面模型 数据
        if(true) {
            PPModelService service = (PPModelService)ServiceGlobal.getService(PPModelService.class,this.getSessionFactory());
            service.testRemoveByPortalPage(et);
        }
    ...
    protected void onCheckEntity(boolean bBaseMode,PortalPage  et, boolean bCreate, boolean bTempMode,EntityError entityError) throws Exception {
        ...
        //检查属性 支持上下文参数
        entityFieldError = onCheckField_EnableCTX( bBaseMode,  et,  bCreate,  bTempMode);
        ...
        //检查属性 门户页面逻辑名称
        entityFieldError = onCheckField_PortalPageMemo( bBaseMode,  et,  bCreate,  bTempMode);
        if(entityFieldError!=null) {
            entityError.register(entityFieldError);
        }
        //检查属性 门户页面名称
    ...
    protected EntityFieldError onCheckField_EnableCTX(boolean bBaseMode,PortalPage et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            ...
            //检查值规则[默认规则]
            strRuleInfo =onTestValueRule_EnableCTX_Default( et,  bCreate,  bTempMode);
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                EntityFieldError entityFieldError = new EntityFieldError();
                entityFieldError.setFieldName(PortalPage.FIELD_ENABLECTX);
    ...
    protected EntityFieldError onCheckField_Memo(boolean bBaseMode,PortalPage et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            ...
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                EntityFieldError entityFieldError = new EntityFieldError();
                entityFieldError.setFieldName(PortalPage.FIELD_MEMO);
                entityFieldError.setErrorType(EntityFieldError.ERROR_VALUERULE);
                entityFieldError.setErrorInfo(strRuleInfo);
                return entityFieldError;
            }
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    EntityFieldError entityFieldError = new EntityFieldError();
                    entityFieldError.setFieldName(PortalPage.FIELD_PORTALPAGEID);
    ...
    protected EntityFieldError onCheckField_PortalPageMemo(boolean bBaseMode,PortalPage et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            ...
            String strRuleInfo  = null;
            //检查值规则[默认规则]
            strRuleInfo =onTestValueRule_PortalPageMemo_Default( et,  bCreate,  bTempMode);
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                EntityFieldError entityFieldError = new EntityFieldError();
                entityFieldError.setFieldName(PortalPage.FIELD_PORTALPAGEMEMO);
                entityFieldError.setErrorType(EntityFieldError.ERROR_VALUERULE);
                entityFieldError.setErrorInfo(strRuleInfo);
                return entityFieldError;
            }
    ...
    protected EntityFieldError onCheckField_PortalPageName(boolean bBaseMode,PortalPage et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    EntityFieldError entityFieldError = new EntityFieldError();
                    entityFieldError.setFieldName(PortalPage.FIELD_PORTALPAGENAME);
                    entityFieldError.setErrorType(EntityFieldError.ERROR_EMPTY);
                    return entityFieldError;
                }
            }
    ...
    protected String onTestValueRule(String strDEFieldName,String strRule,IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_PORTALPAGEID,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_PortalPageId_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_PORTALPAGENAME,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_PortalPageName_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_CREATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_CREATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateDate_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_UPDATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_UpdateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_UPDATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_UpdateDate_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_PORTALPAGEMEMO,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_PortalPageMemo_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_ENABLECTX,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_EnableCTX_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,PortalPage.FIELD_MEMO,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
    ...
    protected String onTestValueRule_PortalPageId_Default(IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        try {
            if((checkFieldStringLengthRule("PORTALPAGEID", et, bTempMode,null,false,100,true,"内容长度必须小于等于[100]")))
                return null;
    ...
    protected String onTestValueRule_PortalPageMemo_Default(IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        try {
            if((checkFieldStringLengthRule("PORTALPAGEMEMO", et, bTempMode,null,false,200,true,"内容长度必须小于等于[200]")))
                return null;
            return "内容长度必须小于等于[200]";
        } catch(Exception ex) {
            return ex.getMessage();
