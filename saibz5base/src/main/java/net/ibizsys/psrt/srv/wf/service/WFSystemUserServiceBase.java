...
/**
 * 实体[WFSystemUser] 服务对象基类
 */
public abstract class WFSystemUserServiceBase extends net.ibizsys.psrt.srv.PSRuntimeSysServiceBase<WFSystemUser> {
    ...
    public WFSystemUserServiceBase () {
        super();
    ...
    protected void onCheckEntity(boolean bBaseMode,WFSystemUser  et, boolean bCreate, boolean bTempMode,EntityError entityError) throws Exception {
        ...
        //检查属性 工作流系统用户标识
        entityFieldError = onCheckField_WFSystemUserId( bBaseMode,  et,  bCreate,  bTempMode);
        if(entityFieldError!=null) {
            entityError.register(entityFieldError);
        }
        //检查属性 工作流系统用户名称
        entityFieldError = onCheckField_WFSystemUserName( bBaseMode,  et,  bCreate,  bTempMode);
        if(entityFieldError!=null) {
            entityError.register(entityFieldError);
        }
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    ...
                    entityFieldError.setFieldName(WFSystemUser.FIELD_WFSYSTEMUSERID);
            ...
            //检查值规则[默认规则]
            strRuleInfo =onTestValueRule_WFSystemUserId_Default( et,  bCreate,  bTempMode);
    ...
    protected EntityFieldError onCheckField_WFSystemUserName(boolean bBaseMode,WFSystemUser et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    EntityFieldError entityFieldError = new EntityFieldError();
                    entityFieldError.setFieldName(WFSystemUser.FIELD_WFSYSTEMUSERNAME);
                    entityFieldError.setErrorType(EntityFieldError.ERROR_EMPTY);
                    return entityFieldError;
                }
            }
            ...
            String strRuleInfo  = null;
            //检查值规则[默认规则]
            strRuleInfo =onTestValueRule_WFSystemUserName_Default( et,  bCreate,  bTempMode);
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                EntityFieldError entityFieldError = new EntityFieldError();
                entityFieldError.setFieldName(WFSystemUser.FIELD_WFSYSTEMUSERNAME);
                entityFieldError.setErrorType(EntityFieldError.ERROR_VALUERULE);
    ...
    */
    protected void onSyncIndexEntity_WFActor(WFSystemUser et,boolean bRemove) throws Exception {
        ...
        if(bRemove) {
            ...
            dstEntity.set(WFActor.FIELD_WFACTORTYPE,"SYSTEMUSER");
    ...
    protected String onTestValueRule(String strDEFieldName,String strRule,IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_WFSYSTEMUSERID,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_WFSystemUserId_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_WFSYSTEMUSERNAME,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_WFSystemUserName_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_CREATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_CREATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateDate_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_UPDATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_UpdateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFSystemUser.FIELD_UPDATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
    ...
    protected String onTestValueRule_WFSystemUserId_Default(IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        try {
            if((checkFieldStringLengthRule("WFSYSTEMUSERID", et, bTempMode,null,false,100,true,"内容长度必须小于等于[100]")))
                return null;
    ...
    protected String onTestValueRule_WFSystemUserName_Default(IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        try {
            if((checkFieldStringLengthRule("WFSYSTEMUSERNAME", et, bTempMode,null,false,200,true,"内容长度必须小于等于[200]")))
                return null;
