...
/**
 * 实体[WFUserGroup] 服务对象基类
 */
public abstract class WFUserGroupServiceBase extends net.ibizsys.psrt.srv.PSRuntimeSysServiceBase<WFUserGroup> {
    ...
    protected void onCheckEntity(boolean bBaseMode,WFUserGroup  et, boolean bCreate, boolean bTempMode,EntityError entityError) throws Exception {
        ...
        //检查属性 用户组名称
        entityFieldError = onCheckField_WFUserGroupName( bBaseMode,  et,  bCreate,  bTempMode);
        if(entityFieldError!=null) {
            entityError.register(entityFieldError);
        }
    ...
    protected EntityFieldError onCheckField_Memo(boolean bBaseMode,WFUserGroup et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            ...
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                ...
                entityFieldError.setFieldName(WFUserGroup.FIELD_MEMO);
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    EntityFieldError entityFieldError = new EntityFieldError();
                    entityFieldError.setFieldName(WFUserGroup.FIELD_WFUSERGROUPID);
    ...
    protected EntityFieldError onCheckField_WFUserGroupName(boolean bBaseMode,WFUserGroup et, boolean bCreate, boolean bTempMode) throws Exception {
        ...
        if(bBaseMode) {
            if(bCreate) {
                if(StringHelper.isNullOrEmpty(value)) {
                    EntityFieldError entityFieldError = new EntityFieldError();
                    entityFieldError.setFieldName(WFUserGroup.FIELD_WFUSERGROUPNAME);
                    entityFieldError.setErrorType(EntityFieldError.ERROR_EMPTY);
                    return entityFieldError;
                }
            }
            ...
            String strRuleInfo  = null;
            //检查值规则[默认规则]
            strRuleInfo =onTestValueRule_WFUserGroupName_Default( et,  bCreate,  bTempMode);
            if(!StringHelper.isNullOrEmpty(strRuleInfo)) {
                EntityFieldError entityFieldError = new EntityFieldError();
                entityFieldError.setFieldName(WFUserGroup.FIELD_WFUSERGROUPNAME);
                entityFieldError.setErrorType(EntityFieldError.ERROR_VALUERULE);
    ...
    */
    protected void onSyncIndexEntity_WFActor(WFUserGroup et,boolean bRemove) throws Exception {
        ...
        if(bRemove) {
            ...
            dstEntity.set(WFActor.FIELD_WFACTORTYPE,"USERGROUP");
    ...
    protected String onTestValueRule(String strDEFieldName,String strRule,IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_WFUSERGROUPID,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_WFUserGroupId_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_WFUSERGROUPNAME,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_WFUserGroupName_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_CREATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_CREATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_CreateDate_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_UPDATEMAN,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_UpdateMan_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_UPDATEDATE,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
            return onTestValueRule_UpdateDate_Default(et,bCreate,bTempMode);
        if((StringHelper.compare(strDEFieldName,WFUserGroup.FIELD_MEMO,true)==0)
                &&(StringHelper.compare(strRule,"DEFAULT",true)==0))
    ...
    protected String onTestValueRule_WFUserGroupId_Default(IEntity et,boolean bCreate,boolean bTempMode) throws Exception {
        try {
            if((checkFieldStringLengthRule("WFUSERGROUPID", et, bTempMode,null,false,100,true,"内容长度必须小于等于[100]")))
                return null;
