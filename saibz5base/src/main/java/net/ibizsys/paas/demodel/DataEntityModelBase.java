...
public abstract class DataEntityModelBase<ET extends IEntity> implements IDataEntityModel<ET>
{
	...
	protected void prepareModels()throws Exception{
		...
		if(this.getDataAccCtrlMode() == DATAACCCTRL_MASTER)
		{
			...
			if(derBases==null)
			{
				throw new Exception(StringHelper.format("实体[%1$s]数据访问控制定义为主实体，但未找到任何关系"));
			}
			...
			while(derBases.hasNext())
			{
				...
				if(iDERBase instanceof IDER1N)
				{
					...
					{
						if(masterDERList==null)
							masterDERList = new  ArrayList<IDER1N> ();
	...
	public void registerDEMainState(IDEMainState iDEMainState){
		if(!StringHelper.isNullOrEmpty(iDEMainState.getId()))
			deMainStateMap.put(iDEMainState.getId(),iDEMainState);
		if(!StringHelper.isNullOrEmpty(iDEMainState.getMSTag()))
			deMainStateMap.put(iDEMainState.getMSTag(),iDEMainState);
...
//	}
	...
	public ISqlCommandModel getCheckKeySqlCommandModel(IDBDialect iDBDialect,boolean bTempMode) throws Exception
	{
		...
		if(bTempMode)
		{
			sql.append("select m1.%2$s from %1$s_tmp m1 where m1.%2$s = ? ",this.getTableName(),this.getKeyDEField().getName());	
		}
		else
		{
			sql.append("select m1.%2$s from %1$s m1 where m1.%2$s = ? ",this.getTableName(),this.getKeyDEField().getName());
		}
	...
	public ISqlCommandModel getCreateSqlCommandModel(IDBDialect iDBDialect,boolean bTempMode)throws Exception
	{
		...
		while(deFields.hasNext())
		{
			...
			//继承属性
			if(iDEField.isInheritDEField())
				continue;
			
			//公式属性
			if(iDEField.isFormulaDEField())
				continue;
			
			//属性不支持临时临时数据
			if(bTempMode && !iDEField.isEnableTempData())
				continue;
			...
			{
				if((StringHelper.compare(iDEField.getPreDefinedType(), IDEField.PREDEFINEDTYPE_CREATEDATE, true)==0)
						||(StringHelper.compare(iDEField.getPreDefinedType(), IDEField.PREDEFINEDTYPE_UPDATEDATE, true)==0))
				{
				...
				if((StringHelper.compare(iDEField.getPreDefinedType(), IDEField.PREDEFINEDTYPE_CREATEMAN, true)==0)
						||(StringHelper.compare(iDEField.getPreDefinedType(), IDEField.PREDEFINEDTYPE_UPDATEMAN, true)==0))
				{
					ProcParam procParam = new ProcParam();
					procParam.setDataType(iDEField.getStdDataType());
...
//		}
	...
	{
		if(masterDERList == null || et ==null)
			return null;
		
		for(IDER1N iDER1N:masterDERList)
		{
			if(et.get(iDER1N.getPickupDEFName())!=null)
				return iDER1N;
		}
	...
	{
		if(orgIdDEField == null|| iEntity == null)
			return null;
