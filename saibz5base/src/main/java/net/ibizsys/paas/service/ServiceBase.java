...
//	private HashMap<Object, ET> cacheEntityMap = new HashMap<Object, ET>();
	...
	protected CallResult internalGet(ET entity,boolean bTryMode) throws Exception
	{
		...
		DBCallResult dbCallResult = getDAO().executeGetSql(null, entity, false);
	...
	public void create(ET et, boolean bGet) throws Exception
	{
		...
		if(this.fillEntityKeyValue(et, false))
		{
			...
			case IService.CHECKKEYSTATE_DELETE:
				throw new ErrorException(Errors.DUPLICATEKEY,StringHelper.format("数据已经被删除，无法再次建立"));
			case IService.CHECKKEYSTATE_EXIST:
				throw new ErrorException(Errors.DUPLICATEKEY,StringHelper.format("数据已经存在，无法再次建立"));
			default:
				break;
			}
		}
	...
	public void save(ET et, int nSaveMode) throws Exception
	{
		...
		{
			if ((nSaveMode & SAVEMODE_CREATEONLY) > 0)
				this.create(et);
			return;
		}
		...
		{
			if ((nSaveMode & SAVEMODE_UPDATEONLY) > 0)
				this.update(et);
			return;
		}

		throw new Exception(StringHelper.format("数据已经被逻辑删除，无法保存"));
	}
	...
	{
		DBCallResult dbCallResult = getDAO().executeCreateSql(null, entity, false);
		if (dbCallResult.isError())
		{
			throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
		}

//		Object objKey = entity.get(this.getDEModel().getKeyDEField().getName());
//		this.cacheEntityMap.remove(objKey);
	}
	...
	{
		DBCallResult dbCallResult = getDAO().executeUpdateSql(null, entity, false);
		if (dbCallResult.isError())
		{
			throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
		}
...
//		this.cacheEntityMap.remove(objKey);
	...
	{
		DBCallResult dbCallResult = getDAO().executeSysUpdateSql(null, entity, false);
		if (dbCallResult.isError())
		{
			throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
		}
	...
	{
		DBCallResult dbCallResult = getDAO().executeRemoveSql(null, et, false);
		if (dbCallResult.isError())
		{
			throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
		}
	...
	public ArrayList<ET> select(ISelectCond iSelectCond2) throws Exception
	{
		...
		this.doServiceWork(new IServiceWork()
		{
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				DBCallResult dbCallResult = getDAO().executeSelectSql(null, iSelectCond, false);
				if (dbCallResult.isError())
				{
					throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
				}
	...
	public ArrayList<ET> selectTemp(ISelectCond iSelectCond2) throws Exception
	{
		...
		this.doServiceWork(new IServiceWork()
		{
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				DBCallResult dbCallResult = getDAO().executeSelectSql(null, iSelectCond, true);
				if (dbCallResult.isError())
				{
					throw new ErrorException(Errors.INTERNALERROR, dbCallResult.getErrorInfo());
				}
	...
	protected void fillParentInfo(ET et) throws Exception
	{
		...
		if (StringHelper.compare(strParentType, WebContext.PARAM_PARENTTYPE_DER1N, true) == 0)
		{
			...
			if (StringHelper.compare(strParentType, WebContext.PARAM_PARENTTYPE_SYSDER1N, true) == 0)
			{
				strTypeParam = WebContext.getDER1NId(this.getWebContext());
			}
	...
	{
		doServiceWork(-1, iServiceWork, false);
	}
	...
	protected void doServiceWork(int nMode, IServiceWork iServiceWork, boolean bTransaction) throws Exception
	{
		long nBeginTime = java.lang.System.currentTimeMillis();
		// 判断当前是否在事物中
		// org.hibernate.Transaction curTransaction = null;
		// if (this.getCurrentSession().getTransaction() == null ||
		// !this.getCurrentSession().getTransaction().isActive())
		// {
		// curTransaction = this.getCurrentSession().beginTransaction();
		// }
	...
	protected DBFetchResult doServiceFetchWork(IDEDataSetFetchContext iDEDataSetFetchContext, String strDEDataSetName, boolean bTempMode) throws Exception
	{
		...
		{
			SessionFactoryManager.addRef();
			DBFetchResult dbFetchResult = getDAO().fetchDEDataSet(iDEDataSetFetchContext, strDEDataSetName, bTempMode);
			SessionFactoryManager.releaseRef(false);
			long nTime = java.lang.System.currentTimeMillis() - nBeginTime;
...
//			ET cacheET = cacheEntityMap.get(objKey);
//			if (cacheET != null)
//			{
//				et.proxy(cacheET);
//				return;
...
//
//			// 不存在
//			cacheET = this.getDEModel().createEntity();
//			cacheET.set(this.getDEModel().getKeyDEField().getName(), objKey);
//			this.get(cacheET);
//			cacheEntityMap.put(objKey, cacheET);
//
//			et.proxy(cacheET);
//			return;
//		}
	...
	protected void setLast(IEntity et, IEntity last, boolean bReplace) throws Exception
	{
		...
		this.doServiceWork(new IServiceWork()
		{
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				...
				if (!bReplace2)
				{
					if (sessionFactorySession.getLastEntity(curEntity) != null)
						return;
				}
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				...
				if (lastEntity2 != null)
				{
					if (lastEntity2 == INVALIDLAST)
					{
						if(bTry2)
							return;
						throw new Exception(StringHelper.format("变更之前数据无效，没有在数据变更之前进行缓存"));
					}
	...
	public int checkKey(ET et3) throws Exception
	{
		...
		{
			if (!this.fillEntityKeyValue(et2, false))
			{
		...
		this.doServiceWork(new IServiceWork()
		{
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				...
				long nTime = java.lang.System.currentTimeMillis() - nBeginTime;
				log.debug(StringHelper.format("checkKey 耗时[%1$s]", nTime));
	 ...
	 * @throws Exception
	 */
	protected boolean checkFieldQueryCountRule(String strFieldName, String strDEDataQueryName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue,
			boolean bIncMaxValue, String strRuleInfo) throws Exception
	{
		return checkFieldQueryCountRule( strFieldName,  strDEDataQueryName,  et,  bTempMode,  nMinValue,  bIncMinValue,  nMaxValue,
			 bIncMaxValue,  strRuleInfo,true);
	}
	 ...
	 */
	protected boolean checkFieldQueryCountRule(String strFieldName, String strDEDataQueryName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue,
			boolean bIncMaxValue, String strRuleInfo,boolean bTryMode) throws Exception
	{
		return checkFieldQueryCountRule2( strFieldName,  strDEDataQueryName,  et,  bTempMode,  nMinValue,  bIncMinValue,  nMaxValue,
			 bIncMaxValue,  strRuleInfo,false,true);
	}
	 ...
	 * @throws Exception
	 */
	protected boolean checkFieldQueryCountRule2(String strFieldName, String strDEDataQueryName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue,
			boolean bIncMaxValue, String strRuleInfo,boolean bAlwaysCheck) throws Exception
	{
		return checkFieldQueryCountRule2( strFieldName,  strDEDataQueryName,  et,  bTempMode,  nMinValue,  bIncMinValue,  nMaxValue,
			 bIncMaxValue,  strRuleInfo,bAlwaysCheck,true);
	}
	 ...
	 */
	protected boolean checkFieldQueryCountRule2(String strFieldName, String strDEDataQueryName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue,
			boolean bIncMaxValue, String strRuleInfo,boolean bAlwaysCheck,boolean bTryMode) throws Exception
	{
		if (bTempMode)
		{
			// 临时数据不检查
		...
		if(!bAlwaysCheck){
			...
			if (last != null)
			{
				...
				if (DataTypeHelper.compare(nStdDataType, objValue, objValue2) == 0)
					return true;
			}
		}
	 ...
	 * @throws Exception
	 */
	protected boolean checkFieldStringLengthRule(String strFieldName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue, boolean bIncMaxValue,
			String strRuleInfo) throws Exception
	{
		return checkFieldStringLengthRule( strFieldName,  et,  bTempMode,  nMinValue,  bIncMinValue,  nMaxValue,  bIncMaxValue,
				 strRuleInfo,true);
	}
	/**
	 * 检查属性字符长度规则
	 ...
	 */
	protected boolean checkFieldStringLengthRule(String strFieldName, IEntity et, boolean bTempMode, Integer nMinValue, boolean bIncMinValue, Integer nMaxValue, boolean bIncMaxValue,
			String strRuleInfo,boolean bTryMode) throws Exception
	{
		if(StringHelper.isNullOrEmpty(strRuleInfo))
			strRuleInfo = "内容长度必须符合范围规则";
		...
		if (objValue != null)
		{
			...
			{
				throw new Exception(StringHelper.format("属性[%1$s]值不是字符类型", strFieldName));
			}
	 ...
	 * @throws Exception
	 */
	protected boolean checkFieldValueRangeRule(String strFieldName, IEntity et, boolean bTempMode, Double fMinValue, boolean bIncMinValue, Double fMaxValue, boolean bIncMaxValue, String strRuleInfo)
			throws Exception
			{
	 ...
	 * @throws Exception
	 */
	protected boolean checkFieldValueRangeRule(String strFieldName, IEntity et, boolean bTempMode, Double fMinValue, boolean bIncMinValue, Double fMaxValue, boolean bIncMaxValue, String strRuleInfo,boolean bTryMode)
			throws Exception
	{
		if(StringHelper.isNullOrEmpty(strRuleInfo))
			strRuleInfo = "数值必须符合值范围规则";
	 ...
	 * @throws Exception
	 */
	protected void logDataChanged(int nEvent,ET et,ET last)throws Exception{
		// 判断当前实体是否日志数据变更
		if ((this.getDEModel().getDataChangeLogMode() == IDataEntity.DATACHGLOG_SINGLEDATA || this.getDEModel().getDataChangeLogMode() == IDataEntity.DATACHGLOG_FULLDATA)
				|| (this.getDEModel().getDataChangeLogMode() != IDataEntity.DATACHGLOG_NONE && nEvent == IService.DATACHGEVENT_DELETE))
		{
		...
		this.doServiceWork(new IServiceWork()
		{
			...
			public void execute(ITransaction iTransaction) throws Exception
			{
				...
				if(nActionMode2 == IService.UPDATEWFINFOMODE_INIT)
				{
					...
					if(nActionMode2 == IService.UPDATEWFINFOMODE_FINISH)
					{
						//结束流程
						finishWF(et);
					}
					else
						if(nActionMode2 == IService.UPDATEWFINFOMODE_CANCELSTART)
						{
							//取消流程启动
							cancelStartWF(et);
						}
	...
	protected void executeActionLogics(String strAction, String strAttachMode, ET et) throws Exception
	{
		...
		/*java.util.Iterator<IDELogicModel<ET>> logics = this.getDEModel().getDEActionLogics(strAction, strAttachMode);
		if (logics != null)
		{
			while (logics.hasNext())
			{
		...
		java.util.Iterator<IDEActionLogicModel> logics2 = this.getDEModel().getDEActionLogics2(strAction, strAttachMode);
		if (logics2 != null)
		{
			while (logics2.hasNext())
			{
	...
	protected String internalImportModel(JSONObject jo) throws Exception
	{
		try
		{
			...
			// String strValue = jo.optString("srfvalue", "");
			if (joValue == null)
			{
				String strValue = "";
				...
				// String strSQLPatch = jo.optString("SRFSQLPATCH", "");
				// if (StringHelper.compare(strSQLPatch, "TRUE", true) == 0)
				// {
				// String strDBType = jo.optString("DBTYPE", "");
				//
				// if (StringHelper.isNullOrEmpty(strDBType) ||
				// (StringHelper.compare(strDBType,
				// this.getDEModel().GetDBType(), true) == 0))
				// {
				// CallResult callResult = new CallResult();
				//
				// String strCheckCode = jo.optString("CHECKCODE", "");
				// String strSQLCode = jo.optString("SQLCODE", "");
				// String strSQLCode2 = jo.optString("SQLCODE2", "");
				//
				// try
				// {
				// // 没有指定数据库类型或数据库类型不一致
				// if (!StringHelper.isNullOrEmpty(strCheckCode))
				// {
				// // 判断对象是否存在
				// ET rowCount = new ET();
				// callResult =
				// BaseDEDataCtrl.SelectSingleEx(this.globalHelperEx,
				// this.getDEModel().GetDBStorage(), strCheckCode, rowCount);
				// if (callResult.getRetCode() != Errors.OK)
				// {
				// log.error(callResult.getErrorInfo());
				// log.debug(strCheckCode);
				// return callResult;
				// }
				//
				...
				// {
				// callResult =
				// BaseDEDataCtrl.ExecuteWithoutResultEx(this.globalHelperEx,
				// this.getDEModel().GetDBStorage(), strSQLCode, null);
				// if (callResult.getRetCode() != Errors.OK)
				// {
				// log.error(callResult.getErrorInfo());
				// log.debug(strSQLCode);
				// return callResult;
				// }
				// }
				//
				// if (!StringHelper.isNullOrEmpty(strSQLCode2))
				// {
				// callResult =
				// BaseDEDataCtrl.ExecuteWithoutResultEx(this.globalHelperEx,
				// this.getDEModel().GetDBStorage(), strSQLCode2, null);
				// if (callResult.getRetCode() != Errors.OK)
				// {
				// log.error(callResult.getErrorInfo());
				// log.debug(strSQLCode2);
				// return callResult;
				// }
				// }
				// }
				...
				// callResult.setErrorInfo(StringHelper.format("执行数据库补丁发生异常，%1$s",
				// ex.getMessage()));
				// log.error(callResult.getErrorInfo(), ex);
				// return callResult;
				// }
				// }
				// }
	...
	protected String onImportCurModel(ET et, JSONObject jo) throws Exception
	{
		...
		{
			throw new Exception(StringHelper.format("[%1$s]数据[%2$s]已经被删除，无法导入", iDEModel.getName(), et.get(iDEModel.getKeyDEField().getName())));
		}
		...
		//
		//
		// callResult = Save(nValue == CHECKKEYSTATE_OK, strActionMode, et);
		// if (callResult.getRetCode() != Errors.OK)
		// {
		// callResult.setUserObject(et);
		// callResult.setErrorInfo(StringHelper.format("[%1$s]数据[%2$s]无法导入，保存失败，%3$s",
		// iDEModel.getName(), et.get(iDEModel.GetMajorDEFHelper().getName()),
	...
	// * @return
	// */
	// protected static CallResult SyncDER1NData(IService iService, String
	// strDER1NId, String strKey, String strDatas)
	// {
	// CallResult callResult = new CallResult();
	//
	// // 判断当前实体是否存在此关系
	...
	// callResult =
	// iService.getGlobalHelper().getDAModelHelper().GetDER1N(strDER1NId,
	// der1N);
	// if (callResult.IsError())
	// return callResult;
	//
	// // 判断主实体与当前实体是否一致
	// IService minorDataCtrl =
	// iService.getGlobalHelper().getDAModelStorage().FindDEDataCtrlEx(der1N.getMINORDEID(),
	// iService);
	// if (minorDataCtrl == null)
	// {
	// callResult.setRetCode(Errors.INTERNALERROR);
	// callResult.setErrorInfo(StringHelper.format("无法获取实体[%1$s]数据访问对象",
	// der1N.getMINORDEID()));
	// return callResult;
	// }
	//
	// ET cond = new ET();
	// cond.SetParamValue(der1N.getMAJORKEYDEFNAME(),
	// minorDataCtrl.getDEModel().GetDEFHelper(der1N.getMAJORKEYDEFNAME()).GetDEFValue(strKey));
	// Vector<ET> der1nDatas = new Vector<ET>();
	//
	// callResult = minorDataCtrl.Select(cond, der1nDatas);
	// if (callResult.IsError())
	// return callResult;
	//
	// Vector<ET> removeDatas = new Vector<ET>();
	// //
	// String strMinorKeyField =
	// minorDataCtrl.getDEModel().GetKeyDEFHelper().getName();
	// for (ET item : der1nDatas)
	// {
	// // 判断数据在不在
	// String strItemKey = item.GetParamStringValue(strMinorKeyField, "");
	// if (!syncDataMap.containsKey(strItemKey))
	...
	//
	// // 执行删除工作
	// if (minorDataCtrl.getDEModel().IsIndexDE())
	// {
	// Vector<DERINDEX> derIndexList = null;
	// TreeMap<String, DERINDEX> derIndexMap = null;
	// TreeMap<String, IService> deDataCtrlMap = null;
	//
	// derIndexList = minorDataCtrl.getDEModel().GetDERINDEXs(true);
	// derIndexMap = new TreeMap<String, DERINDEX>();
	// deDataCtrlMap = new TreeMap<String, IService>();
	// for (DERINDEX derIndex : derIndexList)
	// {
	// derIndexMap.put(derIndex.getTYPEVALUE(), derIndex);
	// }
	//
	// for (ET item : removeDatas)
	// {
	// // 获取类型
	// String strType =
	// item.GetParamStringValue(minorDataCtrl.getDEModel().GetIndexTypeDEFHelper().getName(),
	// "");
	// if (!derIndexMap.containsKey(strType))
	// {
	// callResult.setRetCode(Errors.INTERNALERROR);
	// callResult.setErrorInfo(StringHelper.format("无法识别的索引实体[%1$s]类型值[%2$s]",
	// minorDataCtrl.getDEModel().getId(), strType));
	// return callResult;
	// }
	//
	// IService iRealDataCtrl = null;
	//
	// DERINDEX derIndex = derIndexMap.get(strType);
	// if (deDataCtrlMap.containsKey(derIndex.getDEID()))
	// {
	// iRealDataCtrl = deDataCtrlMap.get(derIndex.getDEID());
	// }
	// else
	// {
	// iRealDataCtrl =
	// minorDataCtrl.getGlobalHelper().getDAModelStorage().FindDEDataCtrlEx(derIndex.getDEID(),
	// minorDataCtrl);
	// deDataCtrlMap.put(derIndex.getDEID(), iRealDataCtrl);
	// }
	// Object objKey = item.GetParamValue(strMinorKeyField);
	// item.SetParamValue(iRealDataCtrl.getDEModel().GetKeyDEFHelper().getName(),
	// objKey);
	//
	...
	protected String getRemoveRejectMsg(String strDERName,String strDERLogicName,String strMajorDEName,String strMinorDEName,String strDataInfo)throws Exception
	{
		...
		return StringHelper.format("%1$s[%2$s]存在关系数据[%3$s]，无法删除！", DEModelGlobal.getDEModel(strMajorDEName).getLogicName(),strDataInfo,strLogicName);
	}
