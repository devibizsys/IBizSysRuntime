...
public class DataSyncBackendService extends BackendServiceBase {
	...
	protected void onStart() throws Exception {
		...
		if (pollTimer == null) {
			...
			pollTimer.schedule(this, nPollTimer, nPollTimer);
		}
	...
	protected void onRun() throws Exception {
		...
		for (IDataSyncOutEngine iDataSyncOutEngine : deDataSyncOutEngines.values()) {
			try {
				...
				selectCond.setOrderInfo("ORDER BY t1.CREATEDATE");
	...
	protected void processDataSyncIns(ArrayList<DataSyncIn> dataSyncInList) throws Exception {
		...
		for (DataSyncIn dataSyncIn : dataSyncInList) {
			...
			while (deDataSyncIns.hasNext()) {
				...
				if (StringHelper.compare(iDEDataSyncIn.getSyncAgent(), dataSyncIn.getSyncAgent(), false) != 0)
					continue;

				IService iService = DEModelGlobal.getDEModel(iDEDataSyncIn.getDataEntity().getName()).getService();
				if (!this.testDataImport(dataSyncIn, iDEDataSyncIn, iService)) {
					if (DataObject.getBoolValue(dataSyncIn.getFileFlag(), false)) {
						// 移除临时文件
						try {
							String strTempFile = DataObject.getStringValue(dataSyncIn, "TEMPFILE", "");
							if (!StringHelper.isNullOrEmpty(strTempFile)) {
								java.io.File file = new java.io.File(strTempFile);
								if (file.exists()) {
									file.delete();
								}
							}
						} catch (Exception ex) {
							log.error(StringHelper.format("删除临时文件发生异常，%1$s", ex.getMessage()), ex);
						}
					}
					continue;
				}
				...
				if (DataObject.getBoolValue(dataSyncIn.getFileFlag(), false)) {
					try {
						String strTempFile = DataObject.getStringValue(dataSyncIn, "TEMPFILE", "");
						String strRealPath = strFileLocalPath + dataSyncIn.getData();

						if (!StringHelper.isNullOrEmpty(strTempFile)) {
							java.io.File file = new java.io.File(strTempFile);
							if (file.exists()) {
								java.io.File dstFile = new java.io.File(strRealPath);
								if (dstFile.exists()) {
									dstFile.delete();
								} else {
									// 判断是否建立目录
									java.io.File path = dstFile.getParentFile();
									path.mkdirs();
								}
								if (!file.renameTo(dstFile)) {
									dataSyncIn2.setError(StringHelper.format("移动文件发生错误"));
								}
							}
						}
					} catch (Exception ex) {
						dataSyncIn2.setError(StringHelper.format("移动文件发生异常，%1$s", ex.getMessage()));
						log.error(dataSyncIn2.getError(), ex);
					}
				} else {

					ActionSessionManager.openSession();
					try {
						ActionSessionManager.getCurrentSession().setActionParam("SRFDATASYNCIN", dataSyncIn);
						processDataImport(dataSyncIn, iDEDataSyncIn, iService);
					} catch (Exception ex) {
