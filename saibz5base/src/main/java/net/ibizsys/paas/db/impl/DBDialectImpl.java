	...
	public String getOrgDRCond(UserRoleData userRoleData,Org curOrg,String strAlias) throws Exception
	{
		...
		if((nOrgDR & IDEDataRange.ORG_PARENT)>0)
		{
			if(bFirst)
			{
				...
				orgCondSb.append(" OR ");
			orgCondSb.append(" %1$s = 1 ",getFuncSQL(
					IDBDialect.FUNC_INSTR,new String[]{StringHelper.format("'%1$s'",curOrg.getLevelCode()), "o1.LEVELCODE"}));
		...
		if((nOrgDR & IDEDataRange.ORG_CHILD)>0)
		{
			...
			orgCondSb.append(" %1$s = 1 ",getFuncSQL(
					IDBDialect.FUNC_INSTR,new String[]{"o1.LEVELCODE",StringHelper.format("'%1$s'",curOrg.getLevelCode())}));
		}
	...
	public String getOrgSecDRCond(UserRoleData userRoleData,OrgSector curOrgSector,String strAlias) throws Exception
	{
		...
		if((nSecDR & IDEDataRange.SECTOR_PARENT)>0)
		{
			if(bFirst)
			{
				...
				orgSecCondSb.append(" OR ");
			orgSecCondSb.append(" %1$s = 1 ",getFuncSQL(
					IDBDialect.FUNC_INSTR,new String[]{StringHelper.format("'%1$s'",curOrgSector.getLevelCode()), "o2.LEVELCODE"}));
		...
		if((nSecDR & IDEDataRange.SECTOR_CHILD)>0)
		{
			if(bFirst)
			{
				...
				orgSecCondSb.append(" OR ");
			orgSecCondSb.append(" %1$s = 1 ",getFuncSQL(
					IDBDialect.FUNC_INSTR,new String[]{"o2.LEVELCODE",StringHelper.format("'%1$s'",curOrgSector.getLevelCode())}));
		}
